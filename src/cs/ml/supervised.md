...
fn main() {
    ...
    let d = 10;
    let X: Array2<f64> = Array2::random_using((n, d), Normal::new(0.,1.).unwrap(), &mut rng);
    let mut w_true = vec![1., -1., 0.5];
    w_true.extend(vec![0.; d-3]);
    let w_true = ndarray::Array1::from(w_true);
    let logits = X.dot(&w_true);
    let prob = logits.mapv(|x| 1.0 / (1.0 + (-x).exp()));
    let y: Vec<u8> = prob.iter().map(|&p| if rng.gen::<f64>() < p {1} else {0}).collect();

    // Train/test split
    let test_size = 0.3;
    let train_size = (n as f64 * (1.0 - test_size)) as usize;
    let (X_tr, X_te) = X.view().split_at(ndarray::Axis(0), train_size);
    let (y_tr, y_te) = y.split_at(train_size);

    let model = LogisticRegression::default()
        .max_iterations(1000)
        .fit(&X_tr.to_owned(), &Array1::from(y_tr.to_vec()))
        .unwrap();

    let y_prob = model.predict_proba(&X_te.to_owned());
    let y_pred = model.predict(&X_te.to_owned());
    let accuracy = y_pred.iter().zip(y_te.iter()).filter(|(a,b)| a == b).count() as f64 / y_te.len() as f64;

    let log_loss = y_te.iter().zip(y_prob.outer_iter()).map(|(&label, prob)| {
        let p = prob[1];
        -((label as f64)*p.ln() + (1.0 - (label as f64))*(1.0 - p).ln())
    }).sum::<f64>() / y_te.len() as f64;

    println!("Accuracy: {:.4}", accuracy);
    println!("Log loss: {:.4}", log_loss);
}
```